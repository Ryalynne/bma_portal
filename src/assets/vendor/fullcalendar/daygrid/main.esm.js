...
const Popover = /** @class */ (function () {
    ...
    Popover.prototype.render = function () {
        ...
        const el = this.el = createElement('div', {
    ...
    Popover.prototype.position = function () {
        ...
        const origin = computeRect(el.offsetParent)
        const clippingRect = computeClippingRect(options.parentEl)
...
const DayGridMirrorRenderer = /** @class */ (function (_super) {
    ...
    DayGridMirrorRenderer.prototype.attachSegs = function (segs, mirrorInfo) {
        ...
        this.dayGrid.rowEls.forEach(function (rowNode, row) {
            const skeletonEl = htmlToElement('<div class="fc-mirror-skeleton"><table></table></div>') // will be absolutely positioned
            let skeletonTopEl
            let skeletonTop
...
const DayTile = /** @class */ (function (_super) {
    ...
    function DayTile(context, el) {
        ...
        const renderFrame = _this.renderFrame = memoizeRendering(_this._renderFrame)
    ...
    DayTile.prototype._renderFrame = function (date) {
        ...
        const title = dateEnv.format(date, createFormatter(this.opt('dayPopoverFormat')) // TODO: cache
        )
        this.el.innerHTML =
...
function renderCellHtml(date, dateProfile, context, otherAttrs) {
    ...
    const isDateValid = rangeContainsMarker(dateProfile.activeRange, date) // TODO: called too frequently. cache somehow.
    const classes = getDayClasses(date, dateProfile, context)
...
const DayGrid = /** @class */ (function (_super) {
    ...
    function DayGrid(context, el, renderProps) {
        ...
        const renderCells = _this.renderCells = memoizeRendering(_this._renderCells, _this._unrenderCells)
    ...
    DayGrid.prototype.renderNumberCellHtml = function (date) {
        const _a = this; const view = _a.view; const dateEnv = _a.dateEnv
        let html = ''
        const isDateValid = rangeContainsMarker(this.props.dateProfile.activeRange, date) // TODO: called too frequently. cache somehow.
        ...
        if (this.renderProps.cellWeekNumbersVisible && (date.getUTCDay() === weekCalcFirstDow)) {
            html += buildGotoAnchorHtml(view, { date, type: 'week' }, { class: 'fc-week-number' }, dateEnv.format(date, WEEK_NUM_FORMAT) // inner HTML
            )
        }
    ...
    DayGrid.prototype.computeRowLevelLimit = function (row) {
        ...
        const trEls = findChildren(this.eventRenderer.rowStructs[row].tbodyEl)
        let i
        let trEl
            ...
            for (i = 0; i < levelSegs.length; i++) {
                ...
                if (totalSegsBelow) { // do we need to replace this segment with one or many "more" links?
                    ...
                    for (j = 0; j < colSegsBelow.length; j++) {
                        moreTd = createElement('td', { className: 'fc-more-cell', rowSpan })
    ...
    DayGrid.prototype.renderMoreLink = function (row, col, hiddenSegs) {
        const _this = this
        const _a = this; const view = _a.view; const dateEnv = _a.dateEnv
        const a = createElement('a', { className: 'fc-more' })
...
const WEEK_NUM_FORMAT$1 = createFormatter({ week: 'numeric' })
...
const DayGridView = /** @class */ (function (_super) {
    ...
    function DayGridView(context, viewSpec, dateProfileGenerator, parentEl) {
        ...
        const dayGridEl = createElement('div', { className: 'fc-day-grid' })

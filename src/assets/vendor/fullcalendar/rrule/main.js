    ...
    ***************************************************************************** */
    ...
    const recurring = {
        parse: function (rawEvent, leftoverProps, dateEnv) {
            if (rawEvent.rrule != null) {
                const props = core.refineProps(rawEvent, EVENT_DEF_PROPS, {}, leftoverProps)
                const parsed = parseRRule(props.rrule, dateEnv)
                if (parsed) {
                    return {
    ...
    function parseRRule(input, dateEnv) {
        ...
        if (typeof input === 'string') {
            ...
            const refined = __assign({}, input) // copy
        ...
        if (rrule$1) {
            return { rrule: rrule$1, allDayGuess }
        }
        return null
    }

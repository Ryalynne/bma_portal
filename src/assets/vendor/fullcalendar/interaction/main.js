        ...
        function AutoScroller() {
            ...
            this.animate = function () {
                if (_this.isAnimating) { // wasn't cancelled between animation calls
                    const edge = _this.computeBestEdge(_this.pointerScreenX + window.pageXOffset, _this.pointerScreenY + window.pageYOffset)
                    if (edge) {
                        const now = getTime()
            ...
            _this.onPointerMove = function (ev) {
                if (_this.isInteracting) { // if false, still waiting for previous drag's revert
                    ...
                    if (!_this.isDistanceSurpassed) {
                        ...
                        const deltaX = ev.deltaX; const deltaY = ev.deltaY
    ...
    */
    const HitDragging = /** @class */ (function () {
        ...
        HitDragging.prototype.processFirstCoord = function (ev) {
            ...
            if (initialHit) {
                if (this.useSubjectCenter && subjectRect) {
                    const slicedSubjectRect = core.intersectRects(subjectRect, initialHit.rect)
        ...
        HitDragging.prototype.queryHitForOffset = function (offsetLeft, offsetTop) {
            const _a = this; const droppableStore = _a.droppableStore; const offsetTrackers = _a.offsetTrackers
            let bestHit = null
        ...
        function DateClicking(settings) {
            ...
            _this.handleDragEnd = function (ev) {
                ...
                if (!pointer.wasTouchScroll) {
                    const _a = _this.hitDragging; const initialHit = _a.initialHit; const finalHit = _a.finalHit
            ...
            const hitDragging = _this.hitDragging = new HitDragging(_this.dragging, core.interactionSettingsToStore(settings))
    ...
    */
    const DateSelecting = /** @class */ (function (_super) {
        ...
        function DateSelecting(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                const _a = _this; const component = _a.component; const dragging = _a.dragging
                const canSelect = component.opt('selectable') &&
            ...
            _this.handleHitUpdate = function (hit, isFinal) {
                const calendar = _this.component.calendar
            ...
            const component = settings.component
            const dragging = _this.dragging = new FeaturefulElementDragging(component.el)
    ...
    const EventDragging = /** @class */ (function (_super) {
        ...
        function EventDragging(settings) {
            ...
            _this.handlePointerDown = function (ev) {
                ...
                const subjectSeg = _this.subjectSeg = core.getElSeg(ev.subjectEl)
            ...
            _this.handleDragStart = function (ev) {
                const initialCalendar = _this.component.calendar
            ...
            _this.handleDragEnd = function (ev) {
                if (_this.isDragging) {
                    const initialCalendar_1 = _this.component.calendar
                    const initialView = _this.component.view
                    const _a = _this; const receivingCalendar = _a.receivingCalendar; const validMutation = _a.validMutation
                    const eventDef = _this.eventRange.def
                    const eventInstance = _this.eventRange.instance
                    const eventApi = new core.EventApi(initialCalendar_1, eventDef, eventInstance)
                    ...
                    if (validMutation) {
                        ...
                        if (receivingCalendar === initialCalendar_1) {
                            ...
                            const transformed = {}
                            for (let _i = 0, _b = initialCalendar_1.pluginSystem.hooks.eventDropTransformers; _i < _b.length; _i++) {
                                const transformer = _b[_i]
                                __assign(transformed, transformer(validMutation, initialCalendar_1))
                            }
                            const eventDropArg = __assign({}, transformed, {
 el: ev.subjectEl,
...
view: initialView
                            ...
                            const dropArg = __assign({}, receivingCalendar.buildDatePointApi(finalHit.dateSpan), { draggedEl: ev.subjectEl, jsEvent: ev.origEvent, view: finalHit.component // should this be finalHit.component.view? See #4644
                             })
            ...
            dragging.touchScrollAllowed = false
            dragging.autoScroller.isEnabled = component.opt('dragScroll')
            const hitDragging = _this.hitDragging = new HitDragging(_this.dragging, core.interactionSettingsStore)
        ...
        EventDragging.prototype.displayDrag = function (nextCalendar, state) {
            const initialCalendar = this.component.calendar
            const prevCalendar = this.receivingCalendar
        ...
        EventDragging.prototype.clearDrag = function () {
            const initialCalendar = this.component.calendar
            const receivingCalendar = this.receivingCalendar
    ...
    function computeEventMutation(hit0, hit1, massagers) {
        ...
        const delta = core.diffDates(date0, date1, hit0.component.dateEnv, hit0.component === hit1.component
    ...
    const EventDragging$1 = /** @class */ (function (_super) {
        __extends(EventDragging, _super)
        function EventDragging(settings) {
            const _this = _super.call(this, settings) || this
            // internal state
            ...
            _this.handleDragEnd = function (ev) {
                const calendar = _this.component.calendar
                const view = _this.component.view
                const eventDef = _this.eventRange.def
                const eventInstance = _this.eventRange.instance
                const eventApi = new core.EventApi(calendar, eventDef, eventInstance)
                const relevantEvents = _this.relevantEvents
                const mutatedRelevantEvents = _this.mutatedRelevantEvents
    ...
    function computeMutation(hit0, hit1, isFromStart, instanceRange, transforms) {
        const dateEnv = hit0.component.dateEnv
        const date0 = hit0.dateSpan.range.start
        const date1 = hit1.dateSpan.range.start
        const delta = core.diffDates(date0, date1, dateEnv, hit0.component.largeUnit)
        const props = {}
        for (let _i = 0, transforms_1 = transforms; _i < transforms_1.length; _i++) {
    ...
    const UnselectAuto = /** @class */ (function () {
        function UnselectAuto(calendar) {
            ...
            this.onDocumentPointerUp = function (pev) {
                const _a = _this; const calendar = _a.calendar; const documentPointer = _a.documentPointer
                ...
                if (!documentPointer.wasTouchScroll) {
                    if (state.dateSelection && // an existing date selection?
                        ...
                        const unselectAuto = calendar.viewOpt('unselectAuto')
                        const unselectCancel = calendar.viewOpt('unselectCancel')
    ...
    */
    const ExternalElementDragging = /** @class */ (function () {
        function ExternalElementDragging(dragging, suppliedDragMeta) {
            ...
            this.handleDragEnd = function (pev) {
                const _a = _this; const receivingCalendar = _a.receivingCalendar; const droppableEvent = _a.droppableEvent
                _this.clearDrag()
                if (receivingCalendar && droppableEvent) {
                    const finalHit = _this.hitDragging.finalHit
                    const finalView = finalHit.component.view
                    const dragMeta = _this.dragMeta
                    const arg = __assign({}, receivingCalendar.buildDatePointApi(finalHit.dateSpan), { draggedEl: pev.subjectEl, jsEvent: pev.origEvent, view: finalView })
            ...
            const hitDragging = this.hitDragging = new HitDragging(dragging, core.interactionSettingsStore)
        ...
        ExternalElementDragging.prototype.canDropElOnCalendar = function (el, receivingCalendar) {
            const dropAccept = receivingCalendar.opt('dropAccept')
    ...
    function computeEventForDateSpan(dateSpan, dragMeta, calendar) {
        ...
        for (let _i = 0, _a = calendar.pluginSystem.hooks.externalDefTransforms; _i < _a.length; _i++) {
            const transform = _a[_i]
            __assign(defProps, transform(dateSpan, dragMeta))
        }
        const def = core.parseEventDef(defProps, dragMeta.sourceId, dateSpan.allDay, calendar.opt('forceEventDuration') || Boolean(dragMeta.duration), // hasEnd
        calendar)
        ...
        const instance = core.createEventInstance(def.defId, { start, end })
        return { def, instance }
    }
    ...
    function getEmbeddedElData(el, name) {
        const prefix = core.config.dataAttrPrefix
    ...
    */
    const ExternalDraggable = /** @class */ (function () {
        function ExternalDraggable(el, settings) {
            ...
            this.handlePointerDown = function (ev) {
                const dragging = _this.dragging
                const _a = _this.settings; const minDistance = _a.minDistance; const longPressDelay = _a.longPressDelay

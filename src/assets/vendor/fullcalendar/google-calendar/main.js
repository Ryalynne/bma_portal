    ...
    const eventSourceDef = {
        parseMeta: function (raw) {
            ...
            if (typeof raw === 'object') {
                const standardProps = core.refineProps(raw, STANDARD_PROPS)
        ...
        fetch: function (arg, onSuccess, onFailure) {
            const calendar = arg.calendar
            const meta = arg.eventSource.meta
            const apiKey = meta.googleCalendarApiKey || calendar.opt('googleCalendarApiKey')
            if (!apiKey) {
                onFailure({
                ...
                const requestParams_1 = buildRequestParams(arg.range, apiKey, meta.data, calendar.dateEnv)
